...
%==============================================
    ...
    if p==1
    subplot(1,2,2);
    line([1,10],[1/sqrt(12),1/sqrt(12)],'Color','red');
    legend('Sample STD','Theoretical STD','Location','southeast');
    subplot(1,2,1)
    line([1,10],[0.5,0.5],'Color','red');
    legend('Sample Mean','Theoretical mean','Location','southeast');
    elseif p==2
    subplot(1,2,2);
    line([1,10],[1,1],'Color','red');
    legend('Sample STD','Theoretical STD','Location','southeast');
    subplot(1,2,1)
    line([1,10],[0,0],'Color','red');
    legend('Sample Mean','Theoretical mean','Location','southeast');
    end
    ...
    for subplot_index=1:length(num_ofsamples)
       ...
       histogram(x,num_ofbins(1),'FaceColor',[0 0.4470 0.7410],'Normalization','pdf');
    ...
    for subplot_index=1:length(num_ofbins)
       ...
       histogram(x,num_ofbins(subplot_index),'FaceColor',[0 0.4470 0.7410],'Normalization','pdf');
...
legend('\fontsize{10}sample std for uniform distr.','\fontsize{10}theoretical std for uniform distr.','Location','east');
set(legend,'color','w');
hold off;
...
plot(samples_axis,sample_m(:,2),'g','Linewidth',1);hold on;
set(gca,'Color','k');
line([1,N],[0,0],'Color','r','Linewidth',1.5);
xlabel('\fontsize{14}sample size'), ylabel('\fontsize{14}Magnitude of sample mean')
ylim([-0.2 0.6]);
title('\fontsize{14}\bfMean vs sample size')
legend('\fontsize{10}sample mean for gaussian distr.','\fontsize{10}theoretical mean for gaussian distr.','Location','east');
set(legend,'color','w');
hold off;

subplot(1,2,2)
%gaussian distribution std
plot(samples_axis,sample_stds(:,2),'g','Linewidth',1);hold on;
line([1,N],[1,1],'color','r','Linewidth',1.5);
set(gca,'Color','k');
xlabel('\fontsize{14}sample size'), ylabel('\fontsize{14}Magnitude of sample std')
ylim([0.2 1.2]);
title('\fontsize{14}\bfStandard deviation vs sample size')
legend('\fontsize{10}sample std for gaussian distr.','\fontsize{10}theoretical std for gaussian distr.','Location','east');
set(legend,'color','w');
hold off;